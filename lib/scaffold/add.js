...
function addConfig(configFilePath, service, done) {
  ...
  fs.readFile(configFilePath, function(err, data) {
    ...
    config.services = Array.from(new Set(config.services));
...
function add(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'acmcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a acmcore-node.json and/or package.json file.')
    );
  }
