...
var bitcoind = require('../').services.Bitcoin({
  node: {
    datadir: process.env.HOME + '/.actinium',
    network: {
      name: 'testnet'
    }
  }
...
bitcoind.start(function(err) {
  ...
  console.log('Actinium Core started');
});

bitcoind.on('ready', function() {

  console.log('Actinium Core ready');
  ...
  async.series([
    function(next) {
      ...
      suite.add('actiniumd getblock (native)', bitcoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('actiniumd getblock (json rpc)', bitcoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('actiniumd gettransaction (native)', bitcoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('actiniumd gettransaction (json rpc)', bitcoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
